...
absence_updater = SmartDBUpdater(
    db_path=DB_PATH,
...
def refresh_absence_analytics_data(date_of_absence):
    ...
    try:
        ...
        from agents.absence_analytics.get_data import refresh_absence_database_from_oracle
...
@tool
def execute_absence_query(sql_query: str, date_of_absence: str = None, user_id: str = None) -> str:
    """
    ...
    if date_of_absence:
        try:
            ...
            logger.warning(f"⚠️  Invalid date format '{date_of_absence}', using today's date")
            absence_date = datetime.now()
    else:
        # Default to today's date
        absence_date = datetime.now()
        logger.info(f"📅 No date provided, using today: {absence_date.strftime('%m-%d-%Y')}")
